...
void Protocol::handle_message(Network &network, std::shared_ptr<asio::ip::tcp::socket> client, SmartBuffer &smartBuffer)
{
    uint8_t opcode;
    smartBuffer >> opcode;
    ...
    {
        smartBuffer.reset();
        smartBuffer << static_cast<uint8_t>(OpCodes::WORLD) << GameConfig::WORLD_WIDTH << GameConfig::WORLD_HEIGHT;
