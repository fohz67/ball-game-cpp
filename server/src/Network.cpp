...
Network::Network(unsigned short port)
    : acceptor(io_context, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port)), port(port) {}
...
void Network::do_accept()
{
    acceptor.async_accept([this](std::error_code ec, asio::ip::tcp::socket socket)
        ...
        if (!ec) {
            ...
            auto client_socket = std::make_shared<asio::ip::tcp::socket>(std::move(socket));
    ...
    socket->async_read_some(asio::buffer(*buffer),
                            [this, socket, buffer](std::error_code ec, std::size_t length)
                            {
                                ...
                                else
                                {
                                    ...
                                    std::cerr << "Client disconnected: " << socket->remote_endpoint() << std::endl;
                                }
                            });
}

void Network::send_to_client(std::shared_ptr<asio::ip::tcp::socket> client, SmartBuffer &smartBuffer)
{
    if (clients.find(client) != clients.end())
    {
        asio::write(*client, asio::buffer(smartBuffer.getBuffer(), smartBuffer.getSize()));
    }
}
